...
*Setup instructions for a gitian build of Forex Trading using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Forex Trading
...
- [Building Forex Trading](#building-forextrading)
...
Clone the git repositories for forextrading and gitian and then checkout the forextrading version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/forextrading-crypto/forextrading.git
cd forextrading
...
Follow the instructions in [doc/release-process.md](release-process.md) in the forextrading repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Forex Trading
----------------

To build Forex Trading (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the forextrading repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/forextrading/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/forextrading-crypto/forextrading
...
For example:
```bash
URL=https://github.com/crowning-/forextrading.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit forextrading=${COMMIT} --url forextrading=${URL} ../forextrading/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit forextrading=${COMMIT} --url forextrading=${URL} ../forextrading/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit forextrading=${COMMIT} --url forextrading=${URL} ../forextrading/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/forextrading-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/forextrading-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/forextrading-build.assert
```
...
[forextrading/gitian.sigs](https://github.com/forextrading-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
