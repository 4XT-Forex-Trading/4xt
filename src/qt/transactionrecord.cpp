...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->z4xtTracker->HasMintTx(hash))) {
            //z4xt stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZ4XT;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zPscs;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by Forex Trading Address
...
bool IsZ4XTType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZ4XT:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZ4XT || type == TransactionRecord::MNReward) {
