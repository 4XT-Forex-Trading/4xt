...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::_4XT);
    ...
    if (!settings.contains("nAnonymizeForexTradingAmount"))
        settings.setValue("nAnonymizeForexTradingAmount", 1000);

    nAnonymizeForexTradingAmount = settings.value("nAnonymizeForexTradingAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeForexTradingAmount"))
        SoftSetArg("-anonymizeforextradingamount", settings.value("nAnonymizeForexTradingAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in forextrading.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeForexTradingAmount:
            return QVariant(nAnonymizeForexTradingAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeForexTradingAmount:
            nAnonymizeForexTradingAmount = value.toInt();
            settings.setValue("nAnonymizeForexTradingAmount", nAnonymizeForexTradingAmount);
            emit anonymizeForexTradingAmountChanged(nAnonymizeForexTradingAmount);
            break;
        case CoinControlFeatures:
