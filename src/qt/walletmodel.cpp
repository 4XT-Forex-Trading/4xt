...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered forextrading address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 4XT.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal forextrading:URI (forextrading:XyZ...?message=example)
            {
...
static void Notifyz4xtReset(WalletModel* walletmodel)
{
    qDebug() << "Notifyz4xtReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->Notifyz4xtReset.connect(boost::bind(Notifyz4xtReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->Notifyz4xtReset.disconnect(boost::bind(Notifyz4xtReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->z4xtTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
