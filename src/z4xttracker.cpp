...
Cz4xtTracker::Cz4xtTracker(std::string strWalletFile)
{
...
Cz4xtTracker::~Cz4xtTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void Cz4xtTracker::Init()
{
...
bool Cz4xtTracker::Archive(CMintMeta& meta)
{
...
bool Cz4xtTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta Cz4xtTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta Cz4xtTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool Cz4xtTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> Cz4xtTracker::GetSerialHashes()
{
...
CAmount Cz4xtTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount Cz4xtTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> Cz4xtTracker::GetMints(bool fConfirmedOnly) const
{
...
bool Cz4xtTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool Cz4xtTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool Cz4xtTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool Cz4xtTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool Cz4xtTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool Cz4xtTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool Cz4xtTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void Cz4xtTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz4xtTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz4xtTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void Cz4xtTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void Cz4xtTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool Cz4xtTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> Cz4xtTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dz4xt from DB\n", __func__, listDeterministicDB.size());
    }
