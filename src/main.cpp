...
#if defined(NDEBUG)
#error "Forex Trading cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in u4XT) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : z4xt spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the z4xt is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 z4xt spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend z4xt without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : z4xt spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : z4xt spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-z4xt");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that z4xt mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 1) {
	...
	else if (nHeight <= 450000 && nHeight > 430000) {
		nSubsidy = 0.018 * COIN;
	}
		...
		else if (nHeight <= 45979750 && nHeight >800000) {
		nSubsidy = 0.011 * COIN;
	}
	else if (nHeight == 45979751) {
		nSubsidy = 0.01 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZ4XTStake)
{
	int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from Forex Trading
...
{
	RenameThread("forextrading-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZ4XTMinted()
{
...
void RecalculateZ4XTSpent()
{
	...
	while (true) {
		...
		//Remove spends from z4xt supply
...
bool Recalculate4XTSupply(int nHeightStart)
{
...
{
	// Forex Trading: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZ4XTSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZ4XTMinted();
		RecalculateZ4XTSpent();
		Recalculate4XTSupply(Params().Zerocoin_StartHeight());
	}

	//Track z4xt money supply in the block index
	if (!UpdateZ4XTSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new z4xt supply for block=%s height=%d", __func__,
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent z4xt spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of z4xt serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: z4xt stake block is less than allowed start height", __func__);

	if (CZPscsStake* z4xt = dynamic_cast<CZPscsStake*>(stake)) {
		CBlockIndex* pindexFrom = z4xt->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with z4xt stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: z4xt stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(z4xt->GetValue()));
		if (nChecksum200 != z4xt->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, z4xt->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZ4XT() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked z4xt fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ForexTradingInputs;
		std::vector<CTxIn> z4xtInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				z4xtInputs.push_back(stakeIn);
			}
			else {
				ForexTradingInputs.push_back(stakeIn);
			}
		}
		const bool hasForexTradingInputs = !ForexTradingInputs.empty();
		const bool hasZForexTradingInputs = !z4xtInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasForexTradingInputs)
					// Check if coinstake input is double spent inside the same block
					for (CTxIn ForexTradingIn : ForexTradingInputs) {
						if (ForexTradingIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			do {
				...
				for (CTransaction t : bl.vtx) {
					for (CTxIn in : t.vin) {
						...
						for (CTxIn stakeIn : ForexTradingInputs) {
			...
			// Now that this loop if completed. Check if we have z4xt inputs.
			if (hasZForexTradingInputs) {
				for (CTxIn zPscsInput : z4xtInputs) {
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-z4xt");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d z4xt mints and %d z4xt spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Forex Trading: We use certain sporks during IBD, so check to see if they are
