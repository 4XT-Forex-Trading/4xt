...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT15000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 15000 4XT at the same time
    ONLY_15000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for z4xt send
enum ZerocoinSpendStatus {
    Z4XT_SPEND_OKAY = 0,                            // No error
    Z4XT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Z4XT_WALLET_LOCKED = 2,                         // Wallet was locked
    Z4XT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Z4XT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Z4XT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Z4XT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Z4XT_TRX_CREATE = 7,                            // Everything related to create the transaction
    Z4XT_TRX_CHANGE = 8,                            // Everything related to transaction change
    Z4XT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Z4XT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Z4XT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Z4XT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Z4XT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Z4XT_SPENT_USED_Z4XT = 14,                      // Coin has already been spend
    Z4XT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    Z4XT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZPscsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZ4XTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fz4xtAuto) const;
    ...
    */
    ...
    Cz4xtWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<Cz4xtTracker> z4xtTracker;
    ...
    void setZWallet(Cz4xtWallet* zwallet)
    {
        zwalletMain = zwallet;
        z4xtTracker = std::unique_ptr<Cz4xtTracker>(new Cz4xtTracker(strWalletFile));
    }

    Cz4xtWallet* getZWallet() { return zwalletMain; }
    ...
    /** z4xt reset */
    boost::signals2::signal<void()> Notifyz4xtReset;
