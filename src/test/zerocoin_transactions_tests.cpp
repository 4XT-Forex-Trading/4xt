...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.z4xtTracker = unique_ptr<Cz4xtTracker>(new Cz4xtTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == Z4XT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == Z4XT_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
