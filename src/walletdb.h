...
// Copyright (c) 2016-2018 The Forex Trading developers
...
public:
    ...
    bool WriteZ4XTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZ4XTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZ4XTSeed_deprecated(uint256& seed);
    bool EraseZ4XTSeed();
    bool EraseZ4XTSeed_deprecated();

    bool WriteZ4XTCount(const uint32_t& nCount);
    bool ReadZ4XTCount(uint32_t& nCount);
