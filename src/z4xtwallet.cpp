...
Cz4xtWallet::Cz4xtWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing z4xt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZ4XTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZ4XTSeed_deprecated()) {
                    LogPrintf("%s: Updated z4xt seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old z4xt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of z4xt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool Cz4xtWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZ4XTCount(nCountLastUsed);
    else if (!walletdb.ReadZ4XTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void Cz4xtWallet::Lock()
{
    seedMaster = 0;
}

void Cz4xtWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void Cz4xtWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZ4XT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool Cz4xtWallet::LoadMintPoolFromDB()
{
...
void Cz4xtWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void Cz4xtWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void Cz4xtWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->z4xtTracker->HasPubcoinHash(pMint.first)) {
...
bool Cz4xtWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZ4XT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to z4xtTracker which also adds to database
    pwalletMain->z4xtTracker->Add(dMint, true);
...
void Cz4xtWallet::SeedToZ4XT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 Cz4xtWallet::GetZerocoinSeed(uint32_t n)
{
...
void Cz4xtWallet::GenerateDeterministicZ4XT(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void Cz4xtWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool Cz4xtWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
